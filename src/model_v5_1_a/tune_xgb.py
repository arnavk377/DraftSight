...
Run:
    python -m src.model_v5_1_a.tune_xgb
"""
...
# No PyTorch ops — allow full CPU parallelism
...
TUNE_LAST_N_FOLDS = 7   # 2018–2024: same as RF grid to get consistent comparison

XGB_GRID = {
    "max_depth":        [3, 4, 6],       # 3 is current; try slightly deeper
...
def run_fold(cfg: dict, X_train, X_test, y_train_orig: np.ndarray) -> tuple[np.ndarray, int]:
    """Train XGB on one fold and return (predictions, best_iteration)."""
...
def main():
    ...
    for test_year in tune_years:
        ...
        # XGB trains on original AV scale (Tweedie loss — no log transform)
