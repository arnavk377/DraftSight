"""
Model v5.1: full data integration from supabase exports + custom roster files.

Data:   supabase_exports/  drafts.csv, college_stats.csv, av.csv
                           draft_pick_context_features.csv  (roster depth cols only)
        data/roster/       veteran_performance_features.csv  (prior-season vet AV/age by pos)
                           draft_positional_context_features.csv  (same-draft team pos context)
                           pick_trade_flags.csv  (was_traded, was_primary_pick, trade_had_players)

Models: Spline Ridge, XGBoost, CatBoost, Random Forest,
...
from src.model_v5_1.data_loader import (
    CFB_STAT_COLS,
    CONTEXT_ROSTER_COLS,
    DRAFT_CAT_COLS,
    DRAFT_NUM_COLS,
...
OUT_DIR = os.path.join(REPO_ROOT, "poc_outputs_v5_1")
...
USE_POSITION_MEDIAN_IMPUTE: bool = True
...
EXCLUDE_COLLEGE_STATS: bool = False
...
def train_embed_model(model, X_num_tr, X_cat_tr, y_tr, desc="Embed", verbose=False, use_huber=False):
    cfg = EMBED_TRAIN_CFG
...
def main():
    ...
    for test_year in tqdm(test_years, desc="Walk-forward folds", unit="yr"):
        ...
        ftt = train_embed_model(ftt, X_num_tr, X_cat_tr, y_train, desc="FTT", use_huber=True)
        ftt.eval()
        with torch.no_grad():
    ...
    results_df.to_csv(os.path.join(OUT_DIR, "model_v5_1_walkforward_results.csv"), index=False)
    summary_df.to_csv(os.path.join(OUT_DIR, "model_v5_1_overall_summary.csv"), index=False)
    selected_df.to_csv(os.path.join(OUT_DIR, "model_v5_1_selected_years_summary.csv"), index=False)
    ...
    if latest_artifacts:
        ...
        latest_pred_df.to_csv(
            os.path.join(OUT_DIR, f"model_v5_1_latest_year_predictions_{year}.csv"), index=False)
