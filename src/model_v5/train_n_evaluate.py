"""
Model v5: supabase-sourced data with the full model variety from v3 and v4's plot style.

Data:   data/supabase_exports/{drafts,college_stats,av,draft_pick_context_features}.csv
        College stats join by pick number — no fuzzy name matching.
        Draft-pick context (roster depth + trade flags) from draft_pick_context_features.csv.

Models: Spline Ridge, XGBoost, CatBoost, Random Forest, LightGBM,
        MLP Embeddings, FT-Transformer, TabNet

Plots:  v4-style grid layouts (walk-forward metrics, pred-vs-actual 2×4,
        overall summary, feature importances, loss curves, pick value curve)

Outputs: poc_outputs_v5/
"""
...
from src.model_v5.ft_transformer import FTTransformer
from src.model_v5.mlp_embeddings import MLPWithEmbeddings
from src.model_v5.tabnet import TabNetRegressor
from src.model_v5.data_loader import (
    CFB_STAT_COLS,
    CONTEXT_NUM_COLS,
    DRAFT_CAT_COLS,
    DRAFT_NUM_COLS,
...
CONTEXT_FEATURES_CSV = os.path.join(SUPABASE_DIR, "draft_pick_context_features.csv")
OUT_DIR = os.path.join(REPO_ROOT, "poc_outputs_v5")
...
TABNET_TRAIN_CFG = dict(
    lr=0.01, lr_decay=0.96, lr_decay_steps=200,
    batch_size=256, max_epochs=100, patience=20, val_fraction=0.15,
)
...
EMBED_TRAIN_CFG = dict(
    lr=1e-3, batch_size=128, max_epochs=100, patience=20, val_fraction=0.15,
)
...
MODEL_NUM_COLS = DRAFT_NUM_COLS + ["college_perf_score", "cfb_matched"] + CFB_STAT_COLS + CONTEXT_NUM_COLS
...
def train_embed_model(model, X_num_tr, X_cat_tr, y_tr, desc="Embed", verbose=False):
    cfg = EMBED_TRAIN_CFG
...
def save_pick_value_curve(spline_model, train_df, scorer, out_path, draft_year):
    ...
    for perf_score, label, color in zip(ref_scores, labels, colors):
        ...
        ax.plot(picks, spline_model.predict(grid), label=label, linewidth=2.0, color=color)
...
def build_model_frame() -> pd.DataFrame:
    ...
    print("Loading draft-pick context features:", CONTEXT_FEATURES_CSV)
    context = load_context_features(CONTEXT_FEATURES_CSV)
    ...
    # Join leakage-safe context features (roster depth + trade flags)
    df = df.merge(context, on=["draft_season", "pick"], how="left")
    ctx_coverage = df["pick_was_traded"].notna().mean() if "pick_was_traded" in df.columns else 0.0
    print(f"Context features join: {ctx_coverage:.1%} picks with trade flags")

    labeled = int(df["av_2yr"].notna().sum())
    print(f"AV label coverage: {labeled}/{len(df)} = {labeled/len(df):.1%}")
...
def main():
    ...
    for test_year in tqdm(test_years, desc="Walk-forward folds", unit="yr"):
        ...
        spline_model = Pipeline([("pre", pre_s), ("reg", Ridge(alpha=4.0))])
        ...
        xgb_model = Pipeline([("pre", pre_x), ("xgb", XGBRegressor(
            n_estimators=400, learning_rate=0.05, max_depth=4,
            min_child_weight=2, subsample=0.85, colsample_bytree=0.85,
            reg_lambda=1.0, nthread=1, random_state=RANDOM_STATE,
        ))])
        ...
        mlpe = train_embed_model(mlpe, X_num_tr, X_cat_tr, y_train, desc="MLPE")
        mlpe.eval()
        with torch.no_grad():
        ...
        ftt = train_embed_model(ftt, X_num_tr, X_cat_tr, y_train, desc="FTT")
        ftt.eval()
        with torch.no_grad():
    ...
    results_df.to_csv(os.path.join(OUT_DIR, "model_v5_walkforward_results.csv"), index=False)
    summary_df.to_csv(os.path.join(OUT_DIR, "model_v5_overall_summary.csv"), index=False)
    selected_df.to_csv(os.path.join(OUT_DIR, "model_v5_selected_years_summary.csv"), index=False)
    ...
    if latest_artifacts:
        ...
        latest_pred_df.to_csv(
            os.path.join(OUT_DIR, f"model_v5_latest_year_predictions_{year}.csv"), index=False)
