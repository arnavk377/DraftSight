...
def load_unlabeled_draft_class(
    draft_year: int,
    drafts_csv: str,
    college_stats_csv: str,
    roster_context_csv: str,
    pick_trade_flags_csv: str,
    pos_context_csv: str,
    vet_perf_csv: str,
) -> pd.DataFrame:
    """Load a draft class without requiring 2-year AV labels.

    Used for predicting future drafts (2025, 2026+) where av_2yr is not yet
    known.  All context feature joins are left-joins; missing rows (e.g. when
    context files have not been rebuilt for the target year) result in NaN
    values that the model's SimpleImputer will fill.

    Returns a DataFrame with the same column structure as build_model_frame()
    except that av_2yr is NaN for all rows.
    """
    draft = load_draft(drafts_csv)
    draft = draft[draft["season"] == draft_year].copy()
    if draft.empty:
        raise ValueError(
            f"No picks for {draft_year} found in {drafts_csv}. "
            "Run `python -m src.data.add_2026_draft` first."
        )

    # College stats (best-effort — may be empty for newest prospects)
    try:
        college_stats = load_college_stats(college_stats_csv)
        draft, join_stats = join_college_stats(draft, college_stats)
        print(f"College stats join: {join_stats}")
    except Exception as exc:
        print(f"College stats unavailable ({exc}); leaving columns as NaN")
        for c in ["college_perf_score", "cfb_matched"]:
            draft[c] = np.nan

    # AV labels: not available for future drafts
    draft = draft.rename(columns={"season": "draft_season"})
    draft["av_2yr"] = np.nan

    # Roster depth context
    try:
        roster_ctx = load_roster_context(roster_context_csv)
        draft = draft.merge(roster_ctx, on=["draft_season", "pick"], how="left")
    except Exception as exc:
    ...
    try:
        ...
        draft = draft.merge(trade_flags, on=["draft_season", "pick"], how="left")
    except Exception as exc:
        print(f"Trade flags unavailable ({exc})")

    # Positional context
    try:
        pos_ctx = load_draft_pos_context(pos_context_csv)
        draft = draft.merge(pos_ctx, on=["draft_season", "pick"], how="left")
    except Exception as exc:
        print(f"Positional context unavailable ({exc})")

    # Veteran performance (joins on prior season = draft_year - 1)
    try:
        vet = load_veteran_performance(vet_perf_csv)
        draft = join_veteran_performance(draft, vet)
        print(f"Veteran performance: {draft['vet_av'].notna().mean():.1%} picks covered")
    except Exception as exc:
        print(f"Veteran performance unavailable ({exc})")

    num_cols_present = [c for c in NUM_COLS if c in draft.columns]
    draft[num_cols_present] = draft[num_cols_present].replace([np.inf, -np.inf], np.nan)
